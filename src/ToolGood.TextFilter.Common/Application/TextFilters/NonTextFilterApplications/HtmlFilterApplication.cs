...
namespace ToolGood.TextFilter.Commonlication
{
    public static class HtmlFilterApplication
    {
        /// <summary>
        /// 查找所有敏感词
        /// </summary>
        ...
        public static IllegalWordsFindAllResult FindAll(in string txt, in bool skipBidi)
        {
            ...
            doc.LoadHtml(txt);
            stream = new HtmlStream(txt, doc);
            stream = translateSearch.Replace(stream, skipBidi);
            var result2 = TextFilterHelper.FindAll(stream);
            stream = null;

            // 4、删除隐藏标签后，检测一次
            stream = new HtmlHideStream(txt, doc);
            doc = null;
            stream = translateSearch.Replace(stream, skipBidi);
            var result4 = TextFilterHelper.FindAll(stream);
            stream = null;

            var result = TextFilterHelper.MergeFindAllResult(new[] { result1, result2, result4 });
            result1 = null;
            result2 = null;
            result4 = null;
            return result;
